...
class _DashboardViewState extends State<DashboardView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (userModel != null && userModel!.userType != UserType.SuperAdmin) {
        var salesRepresentativeDetails = await getStorage.read(StorageKeys.salesRepresentativeDetails);
        salesRepresentativeModel = SalesRepresentativeModel.fromJson(jsonDecode(salesRepresentativeDetails.toString()));
      }

      if (userModel != null) {
        await dashboardController.getOrderList(user: userModel!, salesRepresentative: salesRepresentativeModel);
      }
    });
  }
  ...
  Future<void> onDelete(int orderID) async {
    if (userModel != null) {
      await dashboardController.deleteOrder(
    ...
    return Obx(
          ...
          Scaffold(
            ...
            dashboardController.orderDetailsList.isNotEmpty
              ...
              onPressed: () {
                Get.toNamed(Routes.addEditOrderScreen, arguments: onAddEdit);
              },
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              if(salesRepresentativeModel != null)
                                ...[
                                  SizedBox(height: 4.h),
                                  Text(salesRepresentativeModel!.name, style: TextStyle(fontWeight: FontWeight.w600, color: Colors.black)),
                                ],
                    ...
                    Expanded(
                      child: ListView(
                        ...
                        controller.orderDetailsList.where((element) => element.deletedAt == null).map((order,) {
                          ...
                          if (order.salesRepresentativeID != null &&
                              dashboardController.salesRepresentativeList.isNotEmpty &&
                              dashboardController.salesRepresentativeList.any(
                                    (element) => element.id == order.salesRepresentativeID,
                              ) && salesRepresentativeModel == null) {
                            salesRepresentative = dashboardController.salesRepresentativeList.firstWhere(
                                  (element) => element.id == order.salesRepresentativeID,
                            );
                          }
                          ...
                          return GestureDetector(
                            behavior: HitTestBehavior.translucent,
                            onTap: () {
                              Get.toNamed(Routes.orderDetailsView,arguments: order);
                            },
                            child: Container(
                                          ...
                                          children: [
                                            VerticalTitleValueComponent(
                                              ...
                                              value: salesRepresentative.name +
                                                  (salesRepresentative.isResigned ? ' (Resigned)' : ''),
                                            ),
                                            SizedBox(width: 2),
                                            // if (salesRepresentative.isResigned)
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          GestureDetector(
                                            ...
                                            onTap: () {
                                              if (isEditable) {
                                                Get.toNamed(Routes.addEditOrderScreen, arguments: EditOrderNavigationModel(
                                                  orderID: order.id,
